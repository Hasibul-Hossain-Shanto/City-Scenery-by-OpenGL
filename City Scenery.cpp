...
void Rain(int value)
{
    if (rainday)
    {
        rain += 0.000000001f; // Reduce the increment to slow the rain effect

        glBegin(GL_POINTS);
        for (int i = 1; i <= 300; i++) // Reduce the number of raindrops
        {
            int x = rand(), y = rand();
            x %= 2000;
            y %= 2000;
            ...
            glVertex2d(x, y);
            glVertex2d(x + 5, y + 5); // Adjust raindrop length if needed
            glEnd();
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Shanto Oishik Partho");
